    ...
    public class Engine : IEngine
    {
        ...
        public void Run()
        {
            ...
            if (manufacturer == "HP")
            {
                ...
                pc = new Computer(ComputerType.PC, new BaseCPU(Eight / 4, 32, ram, videoCard), ram, new[] { new HardDriver(500, false, 0) }, videoCard, null);

                var serverRam = new Ram(Eight * 4);
                var serverVideo = new HardDriver();
                server = new Computer(ComputerType.SERVER,
                    new BaseCPU(Eight / 2, 32, serverRam, serverVideo), serverRam,
                    new List<HardDriver>{
                        ...
                        }, serverVideo, null);
                {
                    ...
                    laptop = new Computer(
                        ComputerType.LAPTOP,
                        new BaseCPU(Eight / 4, 64, ram1, card),
                        ram1,
                        new[]
                            {
                                new HardDriver(500,
            ...
            else if (manufacturer == "Dell")
            {
                ...
                pc = new Computer(ComputerType.PC, new BaseCPU(Eight / 2, 64, ram, videoCard), ram, new[] { new HardDriver(1000, false, 0) }, videoCard, null);
                var ram1 = new Ram(Eight * Eight);
                var card = new HardDriver(); server = new Computer(ComputerType.SERVER,
                     new BaseCPU(Eight, 64, ram1, card),
                     ram1,
                     new List<HardDriver>{
                ...
                laptop = new Computer(ComputerType.LAPTOP,
                    new BaseCPU(Eight / 2, ((32)), ram2, videoCard1),
                    ram2,
